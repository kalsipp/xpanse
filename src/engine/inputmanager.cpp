...
void InputManager::read_inputs() {
	...
	while (SDL_PollEvent(&InputManager::m_sdl_event) != 0) {
		...
		//On key up
		case SDL_KEYUP:
			InputManager::set_key(InputManager::m_sdl_event.key.keysym.sym, false);
			break;
		//On key down
		case SDL_KEYDOWN:
			InputManager::set_key(InputManager::m_sdl_event.key.keysym.sym, true);
			break;
		//On mouse up
...
bool InputManager::get_key(int key) {
	if (InputManager::m_keys.count(key) == 0) return false;
...
void InputManager::set_key(int key, bool val) {
	...
	InputManager::m_keys_static[key] = val;
}
