...
void GraphicsManager::initialize() {
	if (!GraphicsManager::m_initialized) {
		...
		else
		{
			...
			//Create window
			GraphicsManager::m_main_window = SDL_CreateWindow( "Let's go bois", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, m_screen_width, m_screen_height, SDL_WINDOW_OPENGL | SDL_WINDOW_RESIZABLE );
			if ( GraphicsManager::m_main_window == NULL )
			{
				std::cout <<  "Window could not be created! SDL_Error:" << SDL_GetError() << std::endl;
				init_ok = false;
			} else {
				GraphicsManager::m_main_renderer = SDL_CreateRenderer(GraphicsManager::m_main_window,
				                                   -1,
